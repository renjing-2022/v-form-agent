  ...
  <el-col class="grid-cell" :class="[customClass]" v-bind="layoutProps" :style="colHeightStyle"
    ...
    <template v-if="!!widget.widgetList && (widget.widgetList.length > 0)">
      <template v-for="(subWidget, swIdx) in widget.widgetList">
        ...
        <template v-else>
          <component :is="getFieldWidget(subWidget.type)" :field="subWidget" :designer="null" :key="swIdx" :parent-list="widget.widgetList"
                     :index-of-parent-list="swIdx" :parent-widget="widget"
                     :sub-form-row-id="subFormRowId" :sub-form-row-index="subFormRowIndex" :sub-form-col-index="subFormColIndex">
            <!-- 递归传递插槽！！！ -->
            <template v-for="slot in Object.keys($slots)" v-slot:[slot]="scope">
...
<script>
  ...
  export default {
    ...
    inject: ['refList', 'globalModel', 'getFormConfig', 'previewState', 'isMixMode', 'getLayoutType'],
    data() {
      return {
        layoutProps: {
      ...
      initLayoutProps() {
        if (!!this.widget.options.responsive) {
          if (!!this.previewState) {
            ...
            let lyType = this.getLayoutType() || this.formConfig.layoutType
            if (lyType === 'H5') {
