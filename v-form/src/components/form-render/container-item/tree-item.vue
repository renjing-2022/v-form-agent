...
<script>
  ...
  export default {
    ...
    methods: {
      initEventHandler() {
        this.on$('setFormDataEx', (newFormData) => {
          if (newFormData && newFormData.hasOwnProperty(this.widget.options.name)) {
            this.setCheckedNodes( newFormData[this.widget.options.name] )
          }
        })
      ...
      setNodeDisabled(data, node) {
        return data.disabled || this.widget.options.disabled
      },
			...
			setTreeData(data, notSetCurrentKey = false) {
				this.widget.options.treeData = data;
				if (data && (data.length > 0) && !notSetCurrentKey) {
