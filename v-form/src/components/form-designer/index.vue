  ...
  <el-container class="main-container full-height">
    ...
    <el-container class="main-content">
      <el-aside v-show="leftAsideVisible" class="side-panel">
        <widget-panel ref="widgetPanelRef" :designer="designer" :key="widgetPanelKey" />
      </el-aside>
...
<script>
  ...
  import {
    ...
    getAllFieldWidgets, traverseAllWidgets, traverseFieldWidgetsOfContainer, setObjectValue, generateId
  } from "@/utils/util"
  ...
  import {
    addFormTemplate,
    clearFormTemplates, deleteFormTemplate,
    getAllFormTemplates
  ...
  export default {
    ...
    provide() {
      return {
        ...
        getServerReferenceFormList: () => this.referenceFormList,
    ...
    created() {
      ...
      this.designer.handleEvent('canvas-select-field', (args) => {
        const fieldWidgetName = args[0]
        const subFormName = args[1]
        this.$emit('field-widget-used', fieldWidgetName, subFormName)
      })
      this.designer.handleEvent('canvas-remove-field', (args) => {
        const fieldWidgetName = args[0]
        const subFormName = args[1]
        this.$emit('field-widget-removed', fieldWidgetName, subFormName)
      })
    ...
    methods: {
      ...
      initFormTemplates() {
        if (!!this.formTemplates) {
          ...
          if (this.formTemplates.length > 0) {
      ...
      initServerFields() {
        if (!!this.fieldListData && !!this.fieldListData.fieldList) {  //优先从fieldListData加载字段名称
          ...
          if (!!this.fieldListData.referenceFormList) {  //加载引用表单名称
            this.referenceFormList.splice(0, this.referenceFormList.length, ...this.fieldListData.referenceFormList)
          }
        } else {
      ...
      setFieldListData(fieldListData) {
        if (!!fieldListData && !!fieldListData.fieldList) {  //优先从fieldListData加载字段名称
          ...
          if (!!fieldListData.referenceFormList) {  //加载引用表单名称
            this.referenceFormList.splice(0, this.referenceFormList.length, ...fieldListData.referenceFormList)
          }
        }
      },
      ...
      addFormTemplate(newFormTemplate) {
        addFormTemplate(newFormTemplate)
        this.designer.emitEvent('refresh-form-templates')
      },

      deleteFormTemplate(templateIndex) {
        deleteFormTemplate(templateIndex)
        this.designer.emitEvent('refresh-form-templates')
      },
