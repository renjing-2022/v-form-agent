...
<template>
  <div class="field-wrapper" :class="{'design-time-bottom-margin': !!this.designer}">
    <el-form-item v-if="!!field.formItemFlag && (!field.options.hidden || (designState === true))"
                  ...
                  :class="[selected ? 'selected' : '', labelHidden, labelAlign, customClass, field.options.required ? 'required' : '', !!field.options.labelWrap ? 'label-wrap' : '']"
  ...
  export default {
    ...
    computed: {
      ...
      labelHidden() {
        // let labelPos = 'left'
        // if (!!this.designer) {
        //   labelPos = this.designer.formConfig.labelPosition
        //   if ((this.designer.formConfig.layoutType === 'H5') && this.designer.formConfig.h5LabelTop) {
        //     labelPos = 'top'
        //   }
        // } else {
        //   labelPos = this.formConfig.labelPosition
        //   if ((this.formConfig.layoutType === 'H5') && this.formConfig.h5LabelTop) {
        //     labelPos = 'top'
        //   }
        // }
        //
        // if ((labelPos === 'top') && this.field.options.labelHidden) {
        //   return 'top-label-hidden'
        // } else {
        //   return ''
        // }

        return this.field.options.labelHidden ? 'label-hidden' : ''
      },
    ...
    methods: {
      ...
      removeFieldWidget() {
        if (!!this.parentList) {
          ...
          const fieldId = this.designer.selectedId
          ...
          this.$nextTick(() => {
            const subFormName = getSubFormNameByFieldId(this.designer.widgetList, fieldId)
            this.parentList.splice(this.indexOfParentList, 1)
            this.designer.setSelected(nextSelected)
            this.designer.formWidget.deleteWidgetRef(fieldRefName, subFormName)  //删除组件ref！！！
            this.designer.emitHistoryChange()
            this.designer.emitEvent('canvas-remove-field', [fieldRefName, subFormName])
          })
        }
      },
