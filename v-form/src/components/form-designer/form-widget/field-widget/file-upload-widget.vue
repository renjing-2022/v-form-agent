    ...
    <el-upload ref="fieldEditor" :disabled="field.options.disabled || isReadMode"
      ...
      <template #tip>
        <div class="el-upload__tip"
             v-if="!!field.options.uploadTip && !isReadMode">{{field.options.uploadTip}}</div>
        <div v-if="isReadMode && noUploadFile">{{i18nt('render.hint.noFile')}}</div>
      </template>
      <template #default>
      ...
      <template #file="{ file }">
        <div class="upload-file-list">
          ...
          <a v-if="showUrl" :href="file.url" download="" target="_blank">
