// 计算公式相关工具方法
import {evalFn, getFieldWidgetById, traverseFieldWidgetsOfContainer} from "@/utils/util";
...
/* 匹配函数入参包含多个字段的情况（应优先匹配），比如：
   {{SUM.SUM.func}}({{number83249.[采购数量].field}}*{{number90572.[采购原价（单价）].field}})
 */
export const FORMULA_ARGS_REG_EXP = /\(((\{\{(\w+\.[^.]+\.\w+)}}).)*/g
...
export function calculateFormula(VFR, DSV, formulaJs, formulaFieldRef, changedFieldRef) {
  ...
  console.log('formulaFieldRef: ', formulaFieldRef)
...
/**
 * 判断字段是否用在计算公式中
 * @param changedFieldId
 * @param formula
 * @param VFR
 */
export function fieldIsUsedInFormula(changedFieldId, formula, VFR) {
  ...
  matchResult.forEach(mi => {
    ...
    if (fieldSchema && (fieldSchema.id === changedFieldId)) {
      foundResult = true
    }
  })
...
export function replaceFieldsAndFunctionsOfFormula(VFR, formulaFieldRef) {
  ...
  matchResult.forEach(mi => {
    ...
    const fieldId = firstPart.substring(2, firstPart.length)
    const fieldSchema = getFieldWidgetById(VFR.formJsonObj.widgetList, fieldId, false)
    const subFormNameOfField = VFR.getSubFormNameByWidgetId(fieldId)
    if (!!fieldSchema ) {
      let fieldRef = VFR.getWidgetRef(fieldSchema.options.name)
      if (!!fieldRef && !subFormNameOfField) {  //主表单字段
        //是否要考虑字符串值的替换？？
...
export function broadcastFormulaRecalculateEvent(subFormRef) {
  let fieldIds = []
  let getFieldIdsFn = (fw) => {
    fieldIds.push(fw.id)
  }
  traverseFieldWidgetsOfContainer(subFormRef.widget, getFieldIdsFn)
  fieldIds.map(fieldId => {
    subFormRef.getFormRef().broadcast('FieldWidget', 'calculate-formula-sfRowDeleted', [null, fieldId])
  })
}
